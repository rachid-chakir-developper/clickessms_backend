...
class Company(models.Model):
    ...
    bank_code = models.CharField("Bank Code", max_length=5, null=True)
    branch_code = models.CharField("Branch Code", max_length=5, null=True)
    account_number = models.CharField("Account Number", max_length=11, null=True)
    rib_key = models.CharField("RIB Key", max_length=2, null=True)
    ...
    def company_admins(self):
        try:
            user_companies = self.company_users.filter(roles__name="ADMIN").select_related('user')
            return [uc.user for uc in user_companies if uc.user]
        except Exception as e:
