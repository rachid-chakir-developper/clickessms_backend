...
class FinanceQuery(graphene.ObjectType):
    ...
    def resolve_bank_accounts(root, info, bank_account_filter=None, offset=None, limit=None, page=None):
        # We can easily optimize query count in the resolve method
        user = info.context.user
        company = user.current_company if user.current_company is not None else user.company
        total_count = 0
        ...
        if bank_account_filter:
            ...
            if keyword:
                bank_accounts = bank_accounts.filter(Q(name__icontains=keyword) | Q(registration_number__icontains=keyword) | Q(driver_name__icontains=keyword))
            if starting_date_time:
...
class CreateBankAccount(graphene.Mutation):
    ...
    def mutate(root, info, image=None, bank_account_data=None):
        ...
        bank_account.company = creator.current_company if creator.current_company is not None else creator.company
        if info.context.FILES:
            # file1 = info.context.FILES['1']
        ...
        bank_account.save()
        folder = Folder.objects.create(name=str(bank_account.id)+'_'+bank_account.name,creator=creator)
...
class UpdateBankAccountState(graphene.Mutation):
    ...
    def mutate(root, info, id, bank_account_fields=None):
        ...
        try:
            bank_account = BankAccount.objects.get(pk=id)
            BankAccount.objects.filter(pk=id).update(is_active=not bank_account.is_active)
            ...
            bank_account=None
            message = "Une erreur s'est produite."
        return UpdateBankAccountState(done=done, success=success, message=message,bank_account=bank_account)
...
class DeleteBankAccount(graphene.Mutation):
    ...
    def mutate(root, info, id):
        ...
        if current_user.is_superuser:
            ...
            message = "Vous n'êtes pas un Superuser."
        return DeleteBankAccount(deleted=deleted, success=success, message=message, id=id)
