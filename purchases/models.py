...
class PurchaseContract(models.Model):
	...
	document = models.ForeignKey('medias.File', on_delete=models.SET_NULL, related_name='purchase_contracts', null=True)
	...
	is_active = models.BooleanField(default=True, null=True)
	folder = models.ForeignKey("medias.Folder", on_delete=models.SET_NULL, null=True)
	supplier = models.ForeignKey("purchases.Supplier", on_delete=models.SET_NULL, related_name="purchase_contracts", null=True)
	company = models.ForeignKey('companies.Company', on_delete=models.SET_NULL, related_name='company_purchase_contracts', null=True)
	creator = models.ForeignKey('accounts.User', on_delete=models.SET_NULL, related_name='purchase_contracts_former', null=True)
	is_deleted = models.BooleanField(default=False, null=True)
	created_at = models.DateTimeField(auto_now_add=True, null=True)
	updated_at = models.DateTimeField(auto_now=True, null=True)
	...
	def save(self, *args, **kwargs):
		...
		super(PurchaseContract, self).save(*args, **kwargs)
	...
	def generate_unique_number(self):
		...
		# Vérifier s'il est unique dans la base de données
		while PurchaseContract.objects.filter(number=number).exists():
			number_suffix = current_time.strftime("%Y%m%d%H%M%S")
			number = f'{number_prefix}{number_suffix}'
